...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, className }) => {
  ...
  return (
    <div className={`exam-paper-preview bg-white text-black font-serif text-[14.5px] leading-relaxed select-text ${className || ''}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          th: ({ children, ...props }) => (
            <th {...props} className="border border-black bg-slate-100 px-2 py-1.5 font-bold text-center text-black">
              {children}
            </th>
          ),
          td: ({ children, ...props }) => (
